...
def main(input_dfs, outdir_intermediate, outdir):
    # Directory where the input dataframes (gapseq's output) are stored
    input_path = "DataFramesFromCluster/" + input_dfs
    print("Input DataFrames folder:", input_path)
    print("Intermediate Output Directory:", outdir_intermediate)
    print("Final Output Directory:", outdir)

    # Merge dataframes
    uptakes_df = read_and_merge_dataframes(input_path, 'uptakes_df')
    mediums_df = read_and_merge_dataframes(input_path, 'mediums_df')
    growth_products_df = read_and_merge_dataframes(input_path, 'growth_products_df')

    # Save the complete medium, uptakes and products dataframes
    outdir_intermediate = outdir_intermediate + "/" + input_dfs
    ...
    genome_table = pd.read_csv("dat/genome_table_protraits", sep="\t", header=None, index_col=1)
    ...
    protraits = pd.read_csv("dat/ProTraits_binaryIntegratedPr0.90.txt", sep="\t",index_col=1)
    ...
    gapseq_to_protraits = pd.read_csv("dat/gapseq_to_protraits.csv", index_col=0).to_dict()["Protraits_id"]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process some dataframes.")
    parser.add_argument("input_dfs", help="Path to input dataframes.")
    parser.add_argument("outdir_intermediate", help="Path to intermediate output directory.")
    parser.add_argument("outdir", help="Path to final output directory.")

    args = parser.parse_args()

    main(args.input_dfs, args.outdir_intermediate, args.outdir)
